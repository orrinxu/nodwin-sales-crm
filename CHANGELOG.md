...
- **Slack notification broadcasts (ORR-771):** the Slack channel in the notification dispatcher was dead code — `sendSlackNotification` queried per-user columns (`slack_user_id`/`access_token`/`user_id`/`enabled`) that don't exist on the per-workspace `slack_connections` table, so it errored at runtime. Reconciled to a **per-workspace incoming-webhook** model (one admin setup, no per-user OAuth): a new **`/admin/slack`** page connects/tests webhooks and picks which events broadcast; `slack_connections` gained a secret `webhook_url` (admin-only SELECT + service-role read, same posture as `email_transport`) + `channel_label`; `sendSlackNotification(message)` now POSTs to every connected webhook (dropped `chat.postMessage`/`SLACK_BOT_TOKEN`). Events: `stage_change`, `deal_won`/`deal_lost`, `deal_assigned`, `approval_requested` (each single-recipient → one post per event). Extends the ORR-706 "deep-links only" stance with low-cost channel broadcasts; the full bot-scoped app (slash command, per-user DMs, per-deal channels, approve-from-Slack) is still a future expansion. +6 sender unit tests, 2-assertion pgTAP.

### Changed

- **Bound `getAccountOptions` to a typeahead-backed 50-row list (perf audit round 2, ORR-767):** the account picker/filter dropdowns loaded the **full** account list (`select id,name order by name`, unbounded) on every contacts / accounts / opportunities page. It's now capped at 50 with every consumer made resilient: the pickers are typeahead-backed (`searchAccountOptions`), and display names are sourced from the record (new `getAccountOptionsByIds`) rather than the bounded list — so nothing drops for accounts outside the cap. Covers the ContactForm primary/link pickers (were static `<select>`s), the contacts-list account filter (was a `<Select>`), and the AccountForm/OpportunityForm pickers (already typeahead). `EntityCombobox.onChange` gained an optional `option` arg so the multi-link add can name a searched account.
- **Gate the dashboard Team panel's fetch on `hasReports` (perf audit round 2, ORR-768):** `dashboard/page.tsx` fetched the Team scorecard + conversion funnel on every load, though they only render for managers (Group was already gated on `canViewGroup`; Team wasn't). `teamScope` still resolves in the parallel wave; the two Team aggregate RPCs now run in a second wave gated on `hasReports` — non-managers skip both with zero added latency, managers pay one extra round-trip. Rendered output is identical.
- **Lazy-load the recharts AI-usage dashboard + the VoiceRecorder chunk (perf audit round 2, ORR-769):** mirroring ORR-760's `*.lazy.tsx` split — `/admin/ai-usage` now dynamic-imports its recharts-heavy dashboard (`ssr:false`), and the record-generator's `VoiceRecorder` (with the `MediaRecorder`/audio chunk) is dynamic-imported so it only loads in the "Record a voice note" dialog step, pulling it out of the opportunities/accounts/contacts route JS.

### Fixed

- **Reporting-chain recursion did a Seq Scan; accounts list sort had no index (perf audit round 2 / index pass, ORR-770):** the db-plans dimension that was blocked during the round-2 sweep, run as structural index-gap analysis (the DB is empty, so `EXPLAIN` timings are moot). Added `idx_users_manager_user_id` (partial) — the reporting-chain recursion (`team_member_ids()`, run every dashboard load, plus visibility-recompute and direct-reports) joined `users` on `manager_user_id` with a Seq Scan, no index existed — and `idx_accounts_created_at` (partial), the exact gap ORR-759 closed for `contacts` but missed for `accounts`. Write-only FKs and lookup FKs were checked and deliberately left unindexed (documented in the migration). 4-assertion pgTAP.

### Changed

- **Bound the per-entity activity feeds (perf audit round 2, ORR-766):** `getActivitiesForOpportunity` / `getActivitiesForAccount` / `getActivitiesForContact` fetched the ENTIRE activity history (wide rows incl. the full `body`, up to 10k chars, + four joins) with no `.limit()`, on every detail-page render, and silently truncated at PostgREST's row cap. All three now take an optional `limit` (default 100) and cap the fetch, matching `getActivities`. The deal copilot (which slices to 8) is unaffected.
- **Request-memoize the currency + pipeline resolution reads (perf audit round 2, ORR-765):** the dashboard and reports pages re-ran the same tiny queries many times per render because the resolvers weren't wrapped in React `cache()` (the pattern `loadUserPreferences` already uses). Now: `resolveOrgReportingCurrency` (~9-20× per render, 2-3 users/settings reads each) and the shared `primary_entity_id` lookup (also used by `getCurrentUserEntityId`) are `cache()`d on `userId`; the tiny `currencies` reference table is loaded once per request (was re-queried ~8-10× per render) and filtered in memory; and the arg-less `pipeline_metrics_agg` RPC is `cache()`d so the dashboard's `getPipelineMetrics` + `getPipelineSummary` share one call instead of two. No behaviour change — `cache()` is request-scoped; it just collapses dozens of duplicate round-trips on the two heaviest pages.
- **CSV importers parallelise their per-row creates + roles count in one query (perf audit, ORR-761, low):** the Salesforce and native CSV importers created each account/contact/opportunity **sequentially, one round-trip per row**. They now run the creates in **bounded-concurrency waves** (a shared `runWithConcurrency`, 20 in flight) after a sequential map/dedupe pass — so N sequential inserts become `ceil(N/20)` waves, while each row is still its own create (validation, within-file dedupe, per-row error attribution and the reported-error cap all preserved). Separately, `getRoles` replaced its N per-role `count(*)` round-trips with a single `role_user_counts()` GROUP BY RPC (3-assertion pgTAP).
- **Client-bundle code-splitting + a trimmed account-page fetch (perf audit, ORR-760):** nothing was `next/dynamic`'d, so heavy libraries shipped eagerly. Now: (1) the `/opportunities` view dynamic-imports `OpportunityBoard` (dnd-kit) and `OpportunityListTable` (tanstack-table) — only one renders per load, so the other's library stays out of the bundle; (2) **recharts** (v3, large) is dynamic-imported with `{ ssr: false }` via lazy wrappers on the dashboard (`PipelineChart`) and reports (`ReportsView` + `ForecastScorecards`), so it loads after paint instead of in the route's initial JS; (3) `accounts/[id]` now builds its relationship-target dropdown from the lightweight `getAccountOptions` instead of `getAccounts` (which fetched every column + a per-row `contact_count` subquery); (4) removed the dead `dashboard-grid.tsx` (365 lines, full dnd-kit — imported only by its own test; the dashboard uses `DashboardTabs`).

### Fixed

- **Opportunity-visibility recompute fired a full recursive rebuild PER ROW (perf audit, ORR-758):** the visibility triggers ran `FOR EACH ROW` on `opportunities` / `opportunity_team_members` / `opportunity_splits`, each doing a DELETE + recursive-UNION rebuild, and the manager-change trigger looped per user × per opportunity — so a bulk import (Salesforce/CSV) or a manager re-org fired a complete recursive rebuild for every affected row. The recompute is now **set-based** (`recompute_visibility_for_opportunities(uuid[])` rebuilds a whole set in one pass, the manager chain carrying its opportunity through one recursive CTE) and the triggers are **statement-level** (transition tables), so one multi-row statement does one rebuild. **The produced `(opportunity_id, user_id, reason)` rows are byte-identical** — verified by snapshotting the old row-level output and diffing it against the new set-based output (0 rows differ either way), plus a new pgTAP covering the bulk-insert, recursion, tier-gating and manager-re-org paths. `SECURITY DEFINER` and the confidential fence are unchanged; the single-row `recompute_visibility_for_opportunity` wrapper is kept for compatibility.
- **Cheap perf wins — missing indexes + an activities RLS short-circuit (perf audit, ORR-759):** added `idx_contacts_created_at (created_at DESC)` (the contacts list default sort had no supporting index) and a partial `idx_accounts_industry (industry) WHERE deleted_at IS NULL` (the account industry `.eq` filter + DISTINCT scan). Reordered the `activities` SELECT policy's OR-chain so the cheap, index-backed `opportunity_visibility` EXISTS and the account-level branch are evaluated **first** — previously the SECURITY DEFINER `can_view_opportunity_by_role_scope()` ran for every activity row before the visibility check could short-circuit. OR is commutative, so the visible set is byte-for-byte identical (verified: an explicit-visibility user still reads a confidential deal's activity; an outsider still reads nothing); only the evaluation order changes, matching every other opportunity-child policy.
- **Remaining JS-over-fetch rollups went silently partial past the row cap (perf audit, ORR-757):** five spots computed a total / DISTINCT / presence by fetching rows and reducing in JS, so they under-reported once a tenant crossed PostgREST's cap. Each now uses a bounded GROUP BY / DISTINCT RPC (SECURITY INVOKER, so RLS still applies), folded through `fetchAndConvert` where money is involved — the `pipeline_metrics_agg` / `forecast_pipeline_agg` pattern: (1) the **report page** (`getReportData`) replaced its `.limit(500)` fetch + JS reduces with `pipeline_metrics_agg` (stages), `report_monthly_agg` (monthly trends) and `report_top_accounts_agg` (top accounts); (2) **stuck-deals** `totalValueAtRisk` now comes from `stuck_deals_value_at_risk` over the whole open set (same stale/overdue predicate, thresholds passed as jsonb) while the displayed list is bounded to the highest-value deals; (3) **account industries** filter uses `distinct_account_industries`; (4) **line-items** presence uses `opportunities_with_line_items` (DISTINCT, bounded by the input page) instead of fetching every line-item row; (5) **notifications** (`getUserNotifications`) gained bounded `.range()` pagination. 9-assertion pgTAP covers the new aggregates + their RLS scoping.
