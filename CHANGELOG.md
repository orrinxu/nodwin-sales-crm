...
- **Facet tabs — fix the tab-bar rule drawing across the tabs (ORR-672 follow-up, #257):** the base `ui/tabs` list is a fixed `h-9` (36px), but the 18px facet tabs are taller, so the list's `border-b` rule rendered *through* the tabs above their true bottom (visible as a faint offset line, esp. on large screens). Override the list to `h-auto items-end` so it grows to the tab height and the rule sits flush under them. Completes the #255 alignment attempt.
