...
| Webhook signature verification | Official SDK from each provider (`@slack/bolt`, `googleapis`). **Postmark exception**: Postmark does not HMAC-sign webhook requests (verified against [Postmark Webhooks API docs](https://postmarkapp.com/developer/api/webhooks-api)); inbound webhook endpoints are authenticated via a static bearer token sent as a custom `X-Postmark-Webhook-Secret` HTTP header, configured in Postmark → Server → Webhooks → HttpHeaders. Token comparison uses `crypto.timingSafeEqual`. | Configuration. Tests proving verification fails with wrong or absent token. NOT signature verification logic. |
| Inbound email parsing | Postmark Inbound (parses the email + DKIM-verifies the sender; sets `DKIMVerified` and `DKIMStatus` in the parsed payload) | The matching logic (which Account, which Opportunity). NOT the email parser, NOT the DKIM check. |
