...
- **Watch for accidentally-committed secrets.** Paperclip's agents will occasionally produce code that embeds a test API key. A gitleaks scan **does** run automatically: `.github/workflows/deploy.yml`'s `checks` job runs the gitleaks binary (against `.gitleaksrc.toml`) on every push to any branch. Note the nuance: `ci.yml` itself has no such job, and the pre-commit hook only runs the RLS linter — so the coverage lives in `deploy.yml`, not the PR CI pipeline. There is no standalone `secret-scan.yml` (it was removed over a license issue). Reviewers should still stay alert to embedded secrets, but the push-time gitleaks gate is the automated backstop.
