...
export async function getAllUsers(ctx: UsersCallContext): Promise<AdminUserRecord[]> {
  ...
  const [{ data: users, error }, { data: entities }, { data: businessUnits }] = await Promise.all([
    supabase
      .from("users")
      .select(
        "id, email, full_name, primary_role, primary_entity_id, primary_business_unit_id, manager_user_id, active, crm_inbound_email",
      )
...
function toDb(input: z.infer<typeof userAdminUpdateSchema>): Record<string, unknown> {
  ...
  if (input.role !== undefined) db.primary_role = input.role
