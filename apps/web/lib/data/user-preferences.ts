...
// Per-request memoized full-row read (ORR-709). The CRM layout reads preferences
// on every page, and several pages (dashboard, opportunities, settings) read them
// again — plus the granular getters below. React cache() dedupes all of that to a
// single round-trip per request, keyed on the primitive userId (NOT the ctx object,
// whose identity differs between the layout and the page). Matches the getMyPermissions
// pattern in lib/security/auth.ts. Scope is per-request, so a preferences upsert in a
// later request is never served a stale cached row.
const loadUserPreferences = cache(
...
export async function getUserPreferences(
  ...
  return loadUserPreferences(ctx.user.id)
}

// The granular getters below resolve one field each. They delegate to the memoized
// full-row read so that, on a page that also loads full preferences (the layout does),
// they cost zero extra round-trips instead of one apiece.

// Resolves just the display currency (used by the dashboards/reports rollup
// path). null means "use the org default".
export async function getDisplayCurrency(
  ctx: UserPreferencesCallContext,
): Promise<string | null> {
  return (await loadUserPreferences(ctx.user.id)).displayCurrency
}

// Resolves just the number-format preference (used by the dashboard number
// formatters). Defaults to "international".
export async function getNumberFormat(
  ctx: UserPreferencesCallContext,
): Promise<NumberFormat> {
  return (await loadUserPreferences(ctx.user.id)).numberFormat
}

// Resolves just the date-format preference (used by the dashboard date
// formatters). Defaults to "iso".
export async function getDateFormat(
  ctx: UserPreferencesCallContext,
): Promise<DateFormat> {
  return (await loadUserPreferences(ctx.user.id)).dateFormat
}
