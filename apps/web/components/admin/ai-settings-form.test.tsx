...
function renderForm(counts: IngestionStatusCounts, failedDocuments: FailedIngestionDocument[]) {
  return render(
    <AiSettingsForm
      settings={settings}
      counts={counts}
...
describe("AiSettingsForm — ingestion failure reasons", () => {
  it("does not render the failure panel when nothing has failed", () => {
    renderForm({ pending: 0, indexed: 3, failed: 0, total: 3 }, [])
  ...
  it("reveals each failed document's stored reason when expanded", async () => {
    ...
    renderForm({ pending: 0, indexed: 1, failed: 1, total: 2 }, docs)
  ...
  it("falls back gracefully when a failed row has no stored message", async () => {
    ...
    renderForm({ pending: 0, indexed: 0, failed: 1, total: 1 }, docs)
    await userEvent.click(screen.getByRole("button", { name: /show failure reason \(1\)/i }))
    expect(screen.getByText(/no error message was recorded/i)).toBeInTheDocument()
  })
  ...
  it("notes truncation when failed count exceeds the fetched list", async () => {
    ...
    renderForm({ pending: 0, indexed: 0, failed: 60, total: 60 }, docs)
