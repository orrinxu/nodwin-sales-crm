...
import { GMAIL_READONLY_SCOPE } from "@/lib/integrations/gmail/gmail-client"

// ORR-832 Gmail pull-sync drain. Invoked by a scheduler (cron → this route),
// because the work makes outbound Gmail API calls that pg_cron cannot. Protected
// by a shared secret (GMAIL_SYNC_CRON_SECRET) — until it is set the route
// reports "not configured" and does nothing. Runs under the service role (no
// user session): it iterates every sync-enabled user with a connected Google
// grant that includes the gmail.readonly scope.
...
function secretOk(provided: string | null): boolean {
  const expected = env.GMAIL_SYNC_CRON_SECRET
 ...
 * users whose `google_gmail_sync_state.sync_enabled` is true AND who hold a
 * connected Google grant that includes the gmail.readonly scope.
 *
...
async function resolveTargetUserIds(
  ...
  const { data: connected, error: connectedError } = await db
    ...
    .select("user_id")
    .eq("status", "connected")
    .contains("granted_scopes", [GMAIL_READONLY_SCOPE])
...
export async function POST(request: NextRequest) {
  if (!env.GMAIL_SYNC_CRON_SECRET) {
    return NextResponse.json(
      { error: "Gmail sync is not configured (GMAIL_SYNC_CRON_SECRET unset)." },
      { status: 503 },
    )
  }
  ...
  try {
    ...
    for (const userId of userIds) {
      try {
        const res = await runGmailSyncForUser(userId)
