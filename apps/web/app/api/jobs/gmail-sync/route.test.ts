...
const SECRET = "test-gmail-cron-secret"
...
vi.mock("@/lib/integrations/gmail/sync", () => ({
  runGmailSyncForUser: (...args: unknown[]) => mockRunSync(...args),
}))
...
const URL_BASE = "https://crm.nodwin.com/api/jobs/gmail-sync"
...
describe("POST /api/jobs/gmail-sync (ORR-832)", () => {
  ...
  it("accepts the secret via x-cron-secret header", async () => {
    mockRunSync.mockResolvedValue({ upserted: 1, scanned: 1 })
    const headers = new Headers()
    headers.set("x-cron-secret", SECRET)
    const req = new Request(`${URL_BASE}?userId=user-7`, {
      method: "POST",
      headers,
  ...
  it("syncs a single user when ?userId= is given", async () => {
    mockRunSync.mockResolvedValue({ upserted: 3, scanned: 5 })
    const res = await POST(postRequest(`${URL_BASE}?userId=user-9`, SECRET))

    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body).toMatchObject({ processed: 1, upserted: 3, scanned: 5, failed: 0 })
    ...
    expect(db.cronInserts[0]).toMatchObject({ job_name: "gmail_sync_drain", status: "ok" })
  })
  ...
  it("iterates sync-enabled + connected users and aggregates the summary", async () => {
    ...
    mockRunSync
      .mockResolvedValueOnce({ upserted: 2, scanned: 4 })
      .mockResolvedValueOnce({ skipped: true, upserted: 0, scanned: 0 })

    const res = await POST(postRequest(URL_BASE, SECRET))

    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body).toMatchObject({ processed: 2, upserted: 2, scanned: 4, skipped: 1 })
